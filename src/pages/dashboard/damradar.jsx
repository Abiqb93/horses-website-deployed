      ...
      <table className="w-full h-full table-auto text-xs border-collapse border border-gray-200"> {/* Adjusted width & height */}
        <thead>
          <tr>
            ...
            <th className="border border-gray-300 py-1 px-2 text-left">{entry2 ? entry2.Sire : "Entry 2 (Select a dam)"}</th>
          </tr>
        </thead>
        <tbody>
...
export function DamRadar() {
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const response = await fetch(`https://horseracesbackend-production.up.railway.app/api/dam_profile?${params.toString()}`);
