    ...
    <Card className="bg-white text-black">
      ...
      <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
        <table className="w-full min-w-[640px] table-auto">
          <thead>
            <tr>
              {["Jockey", "Runners", "Runs", "Winners", "Wins", "WinPercent_", "Stakes_Winners", "Stakes_Wins", "Group_Winners", "Group_Wins"].map((el) => (
...
export function JockeyRadar() {
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/jockey_name_profile?${params.toString()}`);
