...
export function MyDams() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const resDams = await fetch(`http://localhost:8080/api/dam_tracking?user=${userId}`);
        ...
        for (const dam of dams) {
          const response = await fetch(`http://localhost:8080/api/APIData_Table2/dam?damName=${encodeURIComponent(dam.damName)}`);
          ...
          damToHorseMap[dam.damName] = [...new Set(names)];
        }
        ...
        const enrichedDams = dams.map(d => ({
          ...d,
          horseList: damToHorseMap[d.damName] || [],
        }));

        setDamData(enrichedDams);
        setRelatedMap(matchMap);
      } catch (e) {
        console.error(e);
        setError("Failed to fetch dam data or related entries.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async id => {
    if (!window.confirm("Delete this dam tracking entry?")) return;
    try {
      await fetch(`http://localhost:8080/api/dam_tracking/${id}`, { method: "DELETE" });
      setDamData(d => d.filter(r => r.id !== id));
    } catch (e) {
      alert("Delete failed");
    }
  };
    ...
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      {!isLoading && !error && damData.length > 0 && (
        <div className="space-y-6">
          {damData.map((dam) => {
            const horses = dam.horseList || [];
            const visibleHorses = horses.slice(0, visibleCount);

            return (
              <div key={dam.id} className="bg-white rounded-lg shadow p-4">
                <div className="flex justify-between text-sm mb-3">
                  <div>
                    <div><strong>Dam:</strong> {dam.damName}</div>
                    <div><strong>Tracked on:</strong> {new Date(dam.created_at).toLocaleDateString()}</div>
                  </div>
                  <button
      ...
      {!isLoading && !error && damData.length === 0 && (
        <div className="text-gray-500 italic">No dam data available.</div>
      )}
    </div>
  );
}

export default MyDams;
