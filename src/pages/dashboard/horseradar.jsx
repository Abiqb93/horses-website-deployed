    ...
    <Card className="bg-white text-black">
      ...
      <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
        <table className="w-full min-w-[640px] table-auto">
          <thead>
            <tr>
              {["Horse", "Runners", "Runs", "Winners", "Wins", "WinPercent_", "Stakes_Winners", "Stakes_Wins", "Group_Winners", "Group_Wins"].map((el) => (
...
export function HorseRadar() {
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const response = await fetch(`https://horseracesbackend-production.up.railway.app/api/horse_names?${params.toString()}`);
  ...
  return (
    <div className="mt-12 mb-8 flex flex-col gap-12">
      <div className="flex items-center gap-4">
        <label htmlFor="horse-selector" className="text-sm font-medium">
          Select Entry:
        </label>
        <select
          id="horse-selector"
