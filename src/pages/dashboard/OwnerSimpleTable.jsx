// components/OwnerSimpleTable.jsx
...
export default function OwnerSimpleTable({
  baseUrl = "https://horseracesbackend-production.up.railway.app/api",
  rowsPerPage = 3,                 // show 3 per page
  defaultSortBy = "Runners",
  defaultOrder = "desc",
  onOwnerClick,                    // optional: (ownerName, row) => void
  ...
  useEffect(() => { setPage(1); }, [search, sortBy, order]); // reset to page 1 on changes

  // NOTE: backend uses field "Sire" to carry Owner name in owner_profile
  const ownerKey = "Sire";
  ...
  const buildQuery = () => {
    ...
    if (search) p.set("sire", search);        // server expects ?sire= for this dataset
  ...
  async function fetchRows() {
    ...
    try {
      const qs = buildQuery();
      const res = await fetch(`${baseUrl}/owner_profile?${qs}`);
  ...
  const trackOwner = async (ownerFullName) => {
    const storedUser = localStorage.getItem("user");
    const userId = storedUser ? JSON.parse(storedUser).userId : "Guest";
    try {
      // fetch horses by owner
      const res = await fetch(
        `${baseUrl}/APIData_Table2/owner?ownerFullName=${encodeURIComponent(ownerFullName)}`
      );
      ...
      // save tracking
      const postRes = await fetch(`${baseUrl}/owner_tracking`, {
        method: "POST",
  ...
  return (
    <div className="bg-white rounded-lg">
      {/* Search bar — EXACT same style as original */}
                          ...
                          <div className="flex items-center gap-2">
                            {showTrackButton && (
                              <button
                                ...
                                onClick={(e) => { e.stopPropagation(); trackOwner(ownerName); }}
                            ...
                            <span
                              ...
                              onClick={(e) => { e.stopPropagation(); onOwnerClick && onOwnerClick(ownerName, row); }}
                            >
                              {ownerName}
                            </span>
                          </div>
